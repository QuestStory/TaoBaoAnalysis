<template>
    <div class="container">
        <iframe src="/html/loginbackground.html" frameborder="0" width="100%" height="100%"></iframe>
        <div class="login_box">
            <div class="head test">
                淘宝彩妆数据分析平台
            </div>
...
export default {
    ...
    methods: {
        ...
        async login() {
            if(this.login_form.userortel=='17700001234'&&this.login_form.password=='123456'){
                this.$router.push('/manage/screen')
                this.$message({
                    message: '登录成功',
                    type: 'success '                         
                })
            }else{
                console.log(this.login_form.userortel),
                this.$message({
                    message:'账号或者密码错误',
                    type: 'error'
                })
